  ...
  return (
    <div className="flex flex-col mb-24">
      <div className="mb-12 md:mb-16 max-w-xl text-left">
        <h2 className="text-[32px] font-extralight tracking-[-0.02em] mb-4">{title}</h2>
        <div className="text-[#a0a0a0] font-sans text-[15px] leading-relaxed">{description}</div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-12">
        <Panel
          variant="rack"
          screws={false}
          className="p-8 min-h-[400px] flex items-center justify-center relative overflow-hidden"
        >
          {children}

          <div
            style={{ position: 'absolute', bottom: '24px', left: '24px' }}
            className="hidden md:grid grid-cols-8 gap-1 opacity-20 pointer-events-none"
          >
        ...
        <div className="flex flex-col gap-8">
          <Panel variant="rack" screws={false} className="p-6">
            <div className="text-[10px] uppercase tracking-[0.3em] text-[var(--color-accent)] mb-2">
              Component Specs
            </div>
            {specs.map((spec, idx) => (
              <div
