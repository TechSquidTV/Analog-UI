...
Controls render with the default 180 degree light on their own. Wrap a panel or
dense control bank in `AnalogLightingProvider` when controls should share a scene.
Use `interactiveLighting` to let pointer movement and mobile tilt steer the
same light source:
...
export function ConsolePanel() {
  ...
  return (
    <AnalogLightingProvider
      ...
      power={1}
      interactiveLighting={{
        pointer: { enabled: true, surfaceRef: panelRef },
        motion: { enabled: true },
      }}
    >
      <Panel ref={panelRef}>
