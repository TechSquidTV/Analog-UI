...
Controls that participate in provider-level `interactiveLighting` pull in `use-analog-lighting` through their registry dependencies. Install that hook directly only when you are authoring a custom component that should register its own interactive lighting target.
