...
const Screw = ({
  ...
  return (
    <div
      ...
      style={{ '--analog-light-angle-surface': 'var(--analog-light-angle-screw)' } as React.CSSProperties}
    >
      <AnisotropicButton
        variant={variant}
        rotation={rotation}
      ...
      >
        {hole === "slot" && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={filterStyle}>
        ...
        {hole === "star" && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={filterStyle}>
            <svg viewBox="0 0 24 24" className="w-[55%] h-[55%] absolute">
              <g transform={`rotate(${(rotation || 0) - 45} 12 12)`}>
                <path d="M 12 3 L 14.64 8.36 L 20.56 9.22 L 16.28 13.39 L 17.29 19.28 L 12 16.5 L 6.71 19.28 L 7.72 13.39 L 3.44 9.22 L 9.36 8.36 Z" 
                  fill="currentColor" 
                  stroke="currentColor" 
                  strokeWidth="3" 
...
>(({ className, variant = "default", screws = true, screwVariant = "chrome", screwHole = "none", lighting, style, children, ...props }, ref) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        ...style,
        ...(variant === "rack" ? {
          background: `linear-gradient(calc(var(--analog-light-angle-panel, 180deg) - 90deg), rgba(255,255,255,calc(0.05 * var(--analog-light-power, 1))), rgba(255,255,255,0) 40%, rgba(0,0,0,calc(0.3 * var(--analog-light-power, 1)))), #121212`,
          boxShadow: `inset 0 1px 1px rgba(255, 255, 255, calc(0.1 * var(--analog-light-power, 1))), 0 4px 12px rgba(0, 0, 0, 0.5)`
        } : {})
      }}
      {...props}
    >
      {screws && (
        <>
          <Screw variant={screwVariant as "chrome" | "black"} hole={screwHole as "none" | "slot" | "cross" | "star"} className="top-5 left-5" rotation={45} />
          <Screw variant={screwVariant as "chrome" | "black"} hole={screwHole as "none" | "slot" | "cross" | "star"} className="top-5 right-5" rotation={45} />
          <Screw variant={screwVariant as "chrome" | "black"} hole={screwHole as "none" | "slot" | "cross" | "star"} className="bottom-5 left-5" rotation={45} />
          <Screw variant={screwVariant as "chrome" | "black"} hole={screwHole as "none" | "slot" | "cross" | "star"} className="bottom-5 right-5" rotation={45} />
        </>
      )}
      {children}
    </div>
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("flex items-center px-12 pb-12 pt-0", className)}
    {...props}
  />
));
