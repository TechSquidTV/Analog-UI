...
import { Panel, PanelHeader, PanelTitle, PanelDescription, PanelContent, PanelFooter } from './registry/components/analog/Panel';
  ...
  useEffect(() => {
    const tick = () => {
      setChannels(prev => {
        ...
        if (newL >= prev.lPeak) { lPeakRef.current = newL; lPeakTime.current = Date.now(); }
        else if (Date.now() - lPeakTime.current > 1500) { lPeakRef.current = Math.max(newL, lPeakRef.current - 1); }

        if (newR >= prev.rPeak) { rPeakRef.current = newR; rPeakTime.current = Date.now(); }
        else if (Date.now() - rPeakTime.current > 1500) { rPeakRef.current = Math.max(newR, rPeakRef.current - 1); }
...
export default function App() {
  ...
  const [indicatorColor, setIndicatorColor] = useState<'red'|'green'|'amber'|'blue'|'white'>('red');
  ...
  const [meterVariant, setMeterVariant] = useState<'metered' | 'lcd-green' | 'lcd-amber' | 'lcd-blue'>('metered');
  const [isSegmented, setIsSegmented] = useState(true);
  const [gaugeValue, setGaugeValue] = useState(42);
  const [gaugeVariant, setGaugeVariant] = useState<'lcd-green' | 'lcd-amber' | 'lcd-blue'>('lcd-green');
  ...
  return (
    ...
    >
      <div className="w-full min-h-screen p-8 md:p-16 flex flex-col pt-32 md:pt-16">
        ...
        <div className="text-[10px] uppercase tracking-[0.3em] text-[var(--color-accent)] mb-5">Analog Lighting</div>
        ...
        <div className="flex gap-6 items-center">
          ...
          <div className="flex flex-col flex-grow">
            <div className="flex justify-between text-[10px] text-[#888] font-mono uppercase mb-1">
              <span>Angle</span>
              <span className="text-[var(--color-accent)]">{Math.round(((lightAngle % 360) + 360) % 360)}°</span>
            </div>
            ...
            <div className="flex justify-between text-[10px] text-[#888] font-mono uppercase mb-2">
              <span>Track Mouse</span>
              <span className="text-[var(--color-accent)]">{Math.round(mouseInfluence * 100)}%</span>
            </div>
            <input 
              type="range" 
              min="0" 
        ...
        <p className="text-xl text-[var(--color-accent)] font-medium mb-6 italic">UI you can feel.</p>
        <div className="text-[#a0a0a0] font-sans text-[17px] leading-relaxed space-y-4">
          <p>
            Analog UI is a component library and custom registry built on top of shadcn/ui to help you build highly tactile web experiences faster. It provides pre-built "studio hyper-skeuomorphism" components for physical inputs, LED displays, sliders, and more.
          </p>
          <p>
            Components are available via the <code className="bg-[#1a1a1a] text-[#d4d4d4] px-1.5 py-0.5 rounded text-sm font-mono border border-[#333]">shadcn</code> CLI command.
          </p>
        </div>
      </div>
      ...
      >
        <div className="flex flex-col gap-16 items-center justify-center">
          <div className="flex items-center gap-6">
            <span className="font-mono text-xs text-[#555] uppercase tracking-widest">Sys Pwr</span>
            <AnalogToggle 
          ...
          <div className="flex items-center gap-6">
             <span className="font-mono text-xs text-[#555] uppercase tracking-widest">Aux Pwr</span>
             <AnalogToggle 
               variant="black" 
               leftLed={togglesLeftLed} 
               rightLed={togglesRightLed} 
      ...
      >
        <div className="flex flex-col gap-16 items-center justify-center py-12">
          ...
          <div className="flex items-center justify-between w-48 gap-6">
             <span className="font-mono text-xs text-[#555] uppercase tracking-widest">Stealth</span>
             <AnalogSwitch 
               variant="black"
               checked={switch2}
      ...
      <ComponentShowcase 
        ...
        specs={[
          { label: "Animated", value: <AnalogSwitch checked={isMeterAnimated} onCheckedChange={setIsMeterAnimated} /> },
          { label: "Stat L", value: <input type="range" className="w-24 accent-[#555]" disabled={isMeterAnimated} value={staticMeterL} onChange={e => setStaticMeterL(Number(e.target.value))} /> },
          { label: "Stat R", value: <input type="range" className="w-24 accent-[#555]" disabled={isMeterAnimated} value={staticMeterR} onChange={e => setStaticMeterR(Number(e.target.value))} /> },
          { label: "Variant", value: (
            <select className="bg-[#111] text-[#888] border border-[#333] rounded px-2 py-1 text-xs outline-none" value={meterVariant} onChange={e => setMeterVariant(e.target.value as any)}>
          ...
          { label: "Segments", value: <AnalogSwitch checked={isSegmented} onCheckedChange={setIsSegmented} /> }
        ]}
      >
      ...
      <ComponentShowcase 
        ...
        specs={[
          { label: "Value", value: <input type="range" className="w-24 accent-[#555]" value={gaugeValue} onChange={e => setGaugeValue(Number(e.target.value))} /> },
          { label: "Variant", value: (
            <select className="bg-[#111] text-[#888] border border-[#333] rounded px-2 py-1 text-xs outline-none" value={gaugeVariant} onChange={e => setGaugeVariant(e.target.value as any)}>
              <option value="lcd-green">LCD Green</option>
              <option value="lcd-amber">LCD Amber</option>
              <option value="lcd-blue">LCD Blue</option>
            </select>
      ...
      <ComponentShowcase 
        ...
        specs={[
          { label: "Value", value: wheelNum.toString() },
        ]}
      >
        <div className="flex gap-8 items-center justify-center py-12">
           <AnalogWheelNumber 
      ...
      <ComponentShowcase 
        ...
        specs={[
          { label: "State", value: <AnalogSwitch checked={indicatorOn} onCheckedChange={setIndicatorOn} /> },
          { label: "Color", value: (
            <select className="bg-[#111] text-[#888] border border-[#333] rounded px-2 py-1 text-xs outline-none" value={indicatorColor} onChange={e => setIndicatorColor(e.target.value as any)}>
              <option value="red">Red</option>
              <option value="green">Green</option>
          ...
          { label: "Size", value: (
            <select className="bg-[#111] text-[#888] border border-[#333] rounded px-2 py-1 text-xs outline-none" value={indicatorSize} onChange={e => setIndicatorSize(e.target.value as any)}>
          ...
          { label: "Shape", value: (
            <select className="bg-[#111] text-[#888] border border-[#333] rounded px-2 py-1 text-xs outline-none" value={indicatorShape} onChange={e => setIndicatorShape(e.target.value as any)}>
        ...
        <div className="flex gap-16 items-center justify-center py-12">
          <div className="flex flex-col gap-4 items-center">
             <AnalogIndicator 
               ...
               variant="chrome"
             />
             <span className="font-mono text-[10px] text-[#555] uppercase tracking-widest font-bold">Chrome</span>
          </div>
          <div className="flex flex-col gap-4 items-center">
             <AnalogIndicator 
               isOn={indicatorOn}
               ...
               variant="black"
             />
             <span className="font-mono text-[10px] text-[#555] uppercase tracking-widest font-bold">Black</span>
          </div>
        </div>
      </ComponentShowcase>
      ...
      <ComponentShowcase 
        ...
        specs={[
          { label: "Variant", value: (
            <select className="bg-[#111] text-[#888] border border-[#333] rounded px-2 py-1 text-xs outline-none" value={panelVariant} onChange={e => setPanelVariant(e.target.value as any)}>
              <option value="default">Default</option>
              <option value="rack">Rack</option>
            </select>
          )},
          { label: "Screws", value: <AnalogSwitch checked={panelScrews} onCheckedChange={setPanelScrews} /> },
          { label: "Screw Variant", value: (
             <select className="bg-[#111] text-[#888] border border-[#333] rounded px-2 py-1 text-xs outline-none" value={screwVariant} onChange={e => setScrewVariant(e.target.value as any)}>
               <option value="chrome">Chrome</option>
               <option value="black">Black</option>
             </select>
          )},
          { label: "Screw Hole", value: (
             <select className="bg-[#111] text-[#888] border border-[#333] rounded px-2 py-1 text-xs outline-none" value={screwHole} onChange={e => setScrewHole(e.target.value as any)}>
               <option value="none">None</option>
      ...
      >
        <div className="w-full max-w-sm px-4">
          <Panel variant={panelVariant} screws={panelScrews} screwVariant={screwVariant} screwHole={screwHole}>
            <PanelHeader>
            ...
            <PanelContent className="flex flex-col gap-10">
              <div className="flex items-center justify-between">
                ...
                <AnalogToggle value={compressionToggle} onValueChange={setCompressionToggle} className="w-[104px]" />
              </div>
              <div className="flex flex-col gap-10 pt-4">
                ...
                <div className="px-2 pb-2 mt-4">
                  <AnalogSlider value={makeupGain} onValueChange={setMakeupGain} max={12} min={-12} orientation="horizontal" className="w-full min-w-0" />
                </div>
              </div>
            </PanelContent>
            <PanelFooter>
              <button 
                ...
                className={cn(
                  ...
                  bypass ? "bg-[var(--color-accent)] text-white" : "bg-[#222] hover:bg-[#333] text-[#888]"
                )}
              >
                Bypass
              </button>
